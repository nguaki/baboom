...
#               member_recover_password_front_end.php.
#
...
function iCheckIfMemberEmailExists($SafeEmail )
{
	global $mysqli;
...
{	
	...
	$member_email = $InputArray[0];
	$token = $InputArray[1];
	
	$SafeEmail = mysqli_real_escape_string( $mysqli, $member_email );
	$iMemberExists = iCheckIfMemberEmailExists( $SafeEmail );

	if( Token::check( "MEMBER_RECOVER_PW_FORM", $token ) )
	{
		if( $iMemberExists == 1)
		{
			// This assignment is used at member_reset_password.php.
